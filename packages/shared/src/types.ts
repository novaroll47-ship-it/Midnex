...
export interface SpreadRow {
  ...
  cyclesToday?: number;
  /** Ликвидность по стаканам: рекомендуемый объём и прибыль на нём (если стаканы есть). */
  liquidity?: RowLiquidity;
}

/**
 * Что даёт стакан для пары: объём, на котором прибыль в долларах максимальна
 * (не глубже защитного порога чистого спреда), и прибыль на нём. Считается
 * по видимым уровням — без экстраполяции.
 */
export interface RowLiquidity {
  /** Рекомендуемый объём, USDT. */
  recommendedUsdt: number;
  /** Прибыль на рекомендуемом объёме, USDT. */
  profitUsdt: number;
  /** Чистый спред на рекомендуемом объёме, %. */
  netPct: number;
  /** Грязный спред на рекомендуемом объёме, %. */
  grossPct: number;
  /** Рекомендация упёрлась в видимый стакан (мелкий стакан). */
  liquidityCapped: boolean;
  /** Рекомендацию остановил защитный порог чистого спреда. */
  thresholdCapped: boolean;
  /** Какая нога ограничивает ликвидность. */
  limitingLeg: 'long' | 'short' | null;
  /** Стакан неглубокий (топ-20): рекомендация — нижняя оценка. */
  shallow: boolean;
  /** Возраст более старого из двух стаканов, мс. */
  bookAgeMs: number;
}
